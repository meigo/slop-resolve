...
- Fusion FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
   - About this Document
   - Target Audience
   - Requirements
   - Conventions
...
- Fusion FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
   ...
   - TransformMatrix
- 3 Index
- Introduction FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
- Quick Start Tutorial
- Scripting Languages
- Lua
- Python
...
```
```
https://wiki.python.org/moin/Python2orPython
```
```
...
```
```
At the time of writing, the recommended VFX reference platform suggests the latest Python 2.
...
```
```
Tex t Displays the Fusion textedit
...
BezierSpline.AdjustKeyFrames ( _start_ , _end_ , _x_ , _y_ , _operation_ [, _pivotx_ ][, _pivoty_ ])
...
```
BezierSpline.DeleteKeyFrames( _start[, end]_ )
...
> Return type: table
```
BezierSpline.SetKeyFrames( _keyframes[, replace]_ )
...
> Return type: table
```
ChildFrame.SetViewLayout( _layout_ )
...
> Return type: boolean
```
ChildFrame.SwitchControlView( _id_ )
...
FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
```
ChildFrame.SwitchMainView( _id_ )
...
Composition.AddTool _(id[, defsettings][, xpos][, ypos]_ )
...
> Return type: To ol
```
Composition.AddToolAction( _id[, xpos][, ypos]_ )
...
Composition.AskUser( _title, controls_ )
...
> Return type: table
```
Composition.CopySettings( _tool_ )
...
> Return type: table
```
Composition.CopySettings( _toollist_ )
...
> Return type: To ol
```
Composition.FindToolByID( _id_ [, _prev_ ])
...
FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
```
Composition.GetCompPathMap([ _built_ins_ ][, _defaults_ ])
...
```
```
tool.SetData(“Modified.Date”, str(datetim e.no w()))
```
```
...
FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
```
Composition.GetNextKeyTime([ _time_ ][, _tool_ ])
...
> Return type: number
```
Composition.GetPrefs([ _prefname_ ][, _exclude-defaults_ ])
...
> Return type: table
```
Composition.GetPrevKeyTime([ _time_ ][, _tool_ ])
...
> Return type: number
```
Composition.GetPreviewList([ _include_globals_ ])
...
> Return type: table
```
Composition.GetToolList([ _selected_ ][, _regid_ ])
...
> Return type: string
```
Composition.MapPathSegments( _path_ )
...
count ( number ) – count
```
Composition.Render([ _wait_ ][, _start_ ][, _end_ ][, _proxy_ ][, _hiq_ ][, _motionblur_ ])
...
where all the arguments are passed in a table. The table format is useful for declaring non-
contiguos render ranges, such as the following one.
...
> Return type: boolean
```
Composition.ReverseMapPath( _mapped_ )
...
> Return type: string
```
Composition.RunScript( _filename_ )
...
Composition.SetActiveTool( _tool_ )
...
Composition.SetData( _name_ , _value_ )
...
value (( number|string|boolean|table )) – value
```
Composition.SetPrefs( _prefname_ , _val_ )
...
FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
```
FlowView.QueueSetPos( _tool, x, y_ )
...
y ( number ) – y
```
FlowView.Select( _tool_ [, _select_ ])
...
FuFrame.GetPreviewList([ _include_globals_ ])
...
```
Fusion.DumpCgObjects( _filename_ )
...
> Return type: boolean
```
Fusion.DumpGLObjects( _filename_ )
...
> Return type: boolean
```
Fusion.DumpGraphicsHardwareInfo( _filename_ )
...
> Return type: boolean
```
Fusion.DumpOpenCLDeviceInfo( _filename_ )
...
Fusion.GetGlobalPathMap([ _built_ins_ ][, _defaults_ ])
...
Fusion.GetPrefs([ _prefname_ ][, _exclude-defaults_ ])
...
Fusion.GetRegAttrs( _id_ [, _type_ ])
...
> Return type: table
```
Fusion.GetRegSummary( _typemask_ [, _hidden_ ])
...
> Parameters:
typemask ( _number_ ) – typemask
hidden ( _boolean_ ) – hidden
> Returns: regattrs
> Return type: table

Fusion.LoadComp( _filename_ [, _quiet_ ][, _autoclose_ ][, _hidden_ ])
...
> Return type: Composition
```
Fusion.LoadComp( _filename_ , _options_ )
...
> Return type: Composition
```
Fusion.LoadComp( _savedcomp_ , _options_ )
...
FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
```
Fusion.LoadPrefs([ _filename_ ][, _mastername_ ])
...
mastername ( _string_ ) – mastername
> Returns: success
> Return type: boolean

Fusion.LoadRecentComp( _index_ [, _quiet_ ][, _autoclose_ ][, _hidden_ ])
...
the user. Defaults to false.
> Parameters:
index ( _integer_ ) – index
quiet ( _boolean_ ) – quiet
autoclose ( _boolean_ ) – autoclose
hidden ( _boolean_ ) – hidden
> Returns: comp
> Return type: Composition

Fusion.MapPath( _path_ )
...
> Return type: table
```
Fusion.NewComp([ _quiet_ ][, _autoclose_ ][, _hidden_ ])
...
Fusion.OpenFile( _filename_ , _mode_ )
...
Fusion.QueueComp( _filename_ [, _start_ ][, _end_ ][, _group_ ])
...
> Parameters:
exitcode ( _number_ ) – exitcode

Fusion.ReverseMapPath( _mapped_ )
...
FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
```
Fusion.SetPrefs( _prefname_ , _val_ )
...
Fusion.ShowPrefs([ _pageid_ ][, _showall_ ][, _comp_ ])
...
GLImageViewer.EnableLUT([ _enable_ ])
...
> Parameters:
```
enable ( _boolean_ ) – enable

GLImageViewer.EnableRoI([ _enable_ ])
...
> Parameters:
```
pathname ( _string_ ) – pathname

```
> Returns: success
> Return type: boolean
```
GLImageViewer.LockRoI([ _enable_ ])
...
```
GLView.SetLocked( _enable_ )
> Parameters:
enable ( _boolean_ ) – enable

GLView.SetPos( _x, y_ [, _z_ ])
...
scale ( number ) – scale
```
GLView.SetSplit( _x_ , _y_ , _angle_ )
...
angle ( number ) – angle
```
GLView.SetStereoMethod( _method_ [, _o ptio n1_ ][, _option2_ ])
...
option2 – option2
```
GLView.SetStereoSource( _ABsource_ , _stacked_ [, _stackmethod_ ])
...
filename ( string ) – filename
```
GLViewer.SetAlphaOverlayColor( _color_ )
Select which alpha overlay to use.
> Parameters:
color ( _number_ ) – color

GLViewer.SetAspectCorrection( _enable_ )
Enables or disables aspect correction.
> Parameters:
enable ( _boolean_ ) – enable

GLViewer.SetChannel( _channel_ , _toggle_ )
Select which channel to show.
> Parameters:
channel ( _number_ ) – channel
toggle ( _boolean_ ) – toggle


```
FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
```
GLViewer.SetPos( _x_ , _y_ [, _z_ ])
...
scale ( number ) – scale
```
GLViewer.ShowControls( _enable_ )
...
GraphView.DeleteGuides([ _start_ ][, _end_ ])
...
GraphView.GetGuides([ _start_ ][, _end_ ])
Returns a table of snapguide times & names.
> Parameters:
start ( _number_ ) – start
end ( _number_ ) – end
> Returns: guides
> Return type: table
...
FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
```
GraphView.Paste( _desttime_ , _spline1_ [, _spline2..._ ][, _points_ ])
...
s plin e1 ( object ) – spline1
spline2... ( object ) – spline2...
points ( table ) – points
> Returns: success
> Return type: boolean
```
GraphView.SetGuides([ _guides_ ][, _rem_prev_ ])
...
GraphView.ZoomRectangle( _x1_ , _y1_ , _x2_ , _y2_ )
Note: This method is overloaded and has alternative parameters. See other definitions.
...
Link.SetData( _name_ , _value_ )
Set custom persistent data.

See Composition:SetData().
> Parameters:
name ( _string_ ) – name
value (( _number|string|boolean|table_ )) – value
...
Loader.SetMultiClip( _filename_ [, _startframe_ ][, _trimin_ ][, _trimout_ ])
Gives Loader a MultiClip clip list.
> Parameters:
startframe ( _number_ ) – startframe
trimin ( _number_ ) – trimin
trimout ( _number_ ) – trimout
...
MailMessage.AddAttachment( _filename_ )
...
> Return type: boolean
```
MailMessage.AddRecipients( _addresses_ )
...
> Parameters:
bodytext ( _string_ ) – bodytext


```
FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
```
MailMessage.SetHTMLBody( _bodyhtml_ )

Sets the message body using HTML.

> Parameters:
bodyhtml ( _string_ ) – bodyhtml

MailMessage.SetLogin( _login_ )
...
MailMessage.SetPassword( _password_ )
...
MailMessage.SetSender( _sender_ )
...
sender ( table ) – sender
```
MailMessage.SetServer( _servername_ )
Sets the outgoing mail server to use.
> Parameters:
servername ( _string_ ) – servername

MailMessage.SetSubject( _subject_ )
Sets the Subject: field.
> Parameters:
subject ( _string_ ) – subject
...
Operator.AddModifier( _input_ , _modifier_ )
...
> Return type: boolean
```
Operator.ConnectInput( _input_ , _target_ )
...
Operator.FindMainInput( _index_ )
...
> Return type: Input
```
Operator.FindMainOutput( _index_ )
...
> Return type: Output
```
Operator.GetChildrenList([ _selected_ ][, _regid_ ])
...
> Return type: table
```
Operator.LoadSettings( _filename_ )
...
FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
```
Operator.LoadSettings( _settings_ )
...
Operator.SaveSettings( _filename_ )
...
> Return type: boolean
```
Operator.SaveSettings( _customdata_ )
...
> Return type: number
```
Operator.SetData( _name_ , _value_ )
...
value (( number|string|boolean|table )) – value
```
Operator.SetInput( _id_ , _value_ , _time_ )
...
> Parameters:
id ( _string_ ) – id
value (( _number|string|Parameter_ )) – value
time ( _number_ ) – time

Operator.ShowControlPage( _name_ )
...
> Return type: (number|string|boolean|table)
```
Parameter.SetData( _name_ , _value_ )
...
PlainInput.HideViewControls( _hide_ )
...
hide ( boolean ) – hide
```
PlainInput.HideWindowControls( _hide_ )
...
PlainOutput.ClearDiskCache( _start_ , _end_ )
...
PlainOutput.GetDoD([ _time_ ][, _flags_ ][, _proxy_ ])
...
PolylineMask.GetBezierPolyline( _time_ [, _which_ ])
Get a table of bezier polyline.
> Parameters:
time ( _number_ ) – time
which ( _string_ ) – which
> Returns: poly
> Return type: table
...
Preview.Create( _tool_ [, _filename_ ])
...
QueueManager.AddJob( _filename_ [, _groups_ ][, _frames_ ][, _endscript_ ])
...
> Return type: RenderJob
```
QueueManager.AddSlave( _name_ [, _groups_ ][, _unused_ ])
...
QueueManager.LoadQueue( _filename_ )
...
filename ( string ) – filename
```
QueueManager.LoadSlaveList([ _filename_ ])
...
```
QueueManager.MoveJob( _job_ , _offset_ )
...
job ( RenderJob ) – job
```
QueueManager.RemoveSlave( _slave_ )
...
QueueManager.SaveQueue( _filename_ )
...
filename ( string ) – filename
```
QueueManager.SaveSlaveList([ _filename_ ])
...
not want things to be auto-
proxied when it is adjusted.
...
REGB _ Me diaFor mat _C anSaveTex t
...
> Parameters:
slave ( _RenderSlave_ ) – slave


```
FUSION SCRIPTING GUIDE AND REFERENCE MANUAL
```
RenderJob.SetFrames( _frames_ )
...
```
```
RSLVN _ Status number The current status of the slave as number.
```
0. Scanning
1. Idle
2. Failed
...
TimeRegion.FromFrameString( _frames_ )
